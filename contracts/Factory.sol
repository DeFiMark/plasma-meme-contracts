...
interface IHigherFactory {
    ...
    function canCreatePair(address) external view returns (bool);
    function swapEvent(address token0, address token1, address user, uint256 amount0In, uint256 amount1In, uint256 amount0Out, uint256 amount1Out) external;
}
...
interface IHigherPair is IERC20 {
...
contract HigherPair is IHigherPair {
    ...
    string public constant override name = 'HIGHER LPs';
    string public constant override symbol = 'HIGHER-LP';
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= ~uint112(0) && balance1 <= ~uint112(0), 'DEX: OVERFLOW');
...
contract HigherFactory is IHigherFactory {
    bytes32 public constant override INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(HigherPair).creationCode));
    ...
    function canCreatePair(address addr) external view override returns (bool) {
        return _canCreatePair[addr];
    }
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        require(_canCreatePair[msg.sender], 'DEX: FORBIDDEN');

        require(getPair[token0][token1] == address(0), 'DEX: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(HigherPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IHigherPair(pair).initialize(token0, token1);
    ...
    function setCanCreatePair(address addr, bool _canCreatePair_) external {
        require(msg.sender == database.owner(), 'DEX: FORBIDDEN');
        _canCreatePair[addr] = _canCreatePair_;
    }

    function setSwapFee(uint256 _swapFee) external {
        require(msg.sender == database.owner(), 'DEX: FORBIDDEN');
        require(_swapFee <= swapFeeDenominator / 20, 'DEX: INVALID_SWAP_FEE');
        swapFee = _swapFee;
    }
    ...
    function swapEvent(address token0, address token1, address user, uint256 amount0In, uint256 amount1In, uint256 amount0Out, uint256 amount1Out) external override {
        require(isPair[msg.sender], 'DEX: NOT_PAIR');
        bool isWETH0 = token0 == WETH;
        address token = isWETH0 ? token1 : token0;
        bool isBuy = isWETH0 ? amount0In > 0 : amount1In > 0;
        if (isBuy) {
            uint256 quantityETH = isWETH0 ? amount0In : amount1In;
            uint256 quantityToken = isWETH0 ? amount1Out : amount0Out;
            emit Buy(token, user, quantityETH, quantityToken);
        } else {
            uint256 quantityETH = isWETH0 ? amount0Out : amount1Out;
            uint256 quantityToken = isWETH0 ? amount1In : amount0In;
            emit Sell(token, user, quantityETH, quantityToken);
        }
    }
}