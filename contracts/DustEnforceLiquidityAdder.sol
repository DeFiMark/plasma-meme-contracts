...
contract LiquidityAdder is Ownable, ILiquidityAdder {
    ...
    // token slippage
    uint256 public tokenSlippage = 80;
    ...
    function bond(address token) external payable override onlyHigherPumpTokens(token) {
        ...
        if (checkDusted(token) && enforceDust) {
            ...
            if (wethAmountInLP > 0 && tokenAmountInLP == 0) {
                // there is WETH in the LP, but no tokens
                // since sync can't be called yet, we don't have to do anything
                // all this does is boost liquidity and token price on launch, which is good
            } else if (wethAmountInLP == 0 && tokenAmountInLP > 0) {
                // ensure the ratio will match the desired ratio
                // ratio = tokenAmount * 1e18 / wethAmount
                // wethAmount = (tokenAmount * 1e18) / ratio
                uint256 desiredWETHAmount = ( ( tokenAmountInLP * 1e18 ) / desiredRatio );

                // send desiredWETHAmount to dex and sync the LP
                IWETH(WETH).deposit{value: desiredWETHAmount}();
                IERC20(WETH).transfer(pair, desiredWETHAmount);

                // reduce from liquidityAmount
                liquidityAmount -= desiredWETHAmount;
            } else {

                // here, there are two scenarios:
                // more ETH than necessary in LP -> good, leave it
                // more tokens than necessary in LP -> send ETH to even it out

                // determine which asset we need to send
                uint256 currentRatio = ( tokenAmountInLP * 1e18 ) / wethAmountInLP;

                if (currentRatio < desiredRatio) {
                    // more weth than tokens, this is good, leave it
                } else if (currentRatio > desiredRatio) {
                    // price is lower (more tokens per weth), so we need to send WETH to even this out
                    uint256 desiredWETHAmount = ( ( tokenAmountInLP * 1e18 ) / desiredRatio );
                    if (desiredWETHAmount > wethAmountInLP) {
                        uint256 wethToSend = desiredWETHAmount - wethAmountInLP;

                        // send wethToSend to dex and sync the LP
                        IWETH(WETH).deposit{value: wethToSend}();
                        IERC20(WETH).transfer(pair, wethToSend);

                        // reduce from liquidityAmount
                        liquidityAmount -= wethToSend;
                    }
    ...
    function checkDusted(address token) public view returns (bool) {
        ...
        address pair = pairFor(token, WETH);
        return IERC20(WETH).balanceOf(pair) > 0 || IERC20(token).balanceOf(pair) > 0;
    }
